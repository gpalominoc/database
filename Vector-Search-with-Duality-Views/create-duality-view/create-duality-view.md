...
2. Query the view and inspect one movie document.
    ...
    ```sql
    SELECT JSON_SERIALIZE(data PRETTY) AS movie_document
    FROM movies_dv
...
1. Use the duality view in a semantic SQL query and return the top five matching movie documents.
    ...
    The duality view is the application-facing document surface. The query reads the complete movie document from `DATA`, extracts the E5-base vector from the `embedding` field, and compares it with a query embedding generated by the same model. The duality view does not require a second search structure: Oracle can rewrite the vector expression to the underlying `MOVIES.EMBEDDING_E5_BASE` column and use the `SUMMARY_BASE_VEC_IDX` HNSW index.
...
3. Display the execution plan.
    ...
    Look for `VECTOR INDEX HNSW SCAN` with the index name `SUMMARY_BASE_VEC_IDX`. This confirms that the duality view did not prevent Oracle from using the E5-base vector index. The exact plan can vary with table size, statistics, and optimizer settings, so verify the operation and index name in the plan output.
